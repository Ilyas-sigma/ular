var Snake = (function() {
  ...
  var game = {
    ...
    loop: function() {
      ...
      if (!stopped) {
        ...
        ctx.fillText("OJO KENEK BADAN ULO NE", 24, 356);
        ctx.fillText("BUWATAN ASELI ILYAS", 24, 374);
      }
      ...
      if (stopped) {
        ctx.fillStyle = 'rgba(250,250,250,100)';
        ctx.font = "small-caps bold 25px Helvetica";
        ctx.fillText("PENCETEN TOMBOLE BEJIR", 40, 374);
      }

      ctx.fillStyle = 'grey';
      ctx.font = "bold small-caps 18px Helvetica";
      ctx.fillText("ENTUKE APEL: " + points, 150, 40);
      ctx.fillText("JUMLAH PALING AKEH: " + pointsMax, 150, 60);
  ...
  return {
    start: function(fps = 15) {
      window.onload = setup;
      intervalID = setInterval(game.loop, 1650 / fps);
    },
